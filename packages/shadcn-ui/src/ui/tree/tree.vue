...
import type { ClassType, Recordable } from '@yuguaa/typings'

import { onMounted, ref, watch, watchEffect } from 'vue'

import { ChevronRight, IconifyIcon } from '@yuguaa/icons'
import { cn, get } from '@yuguaa/utils'
...
function flatten<T = Recordable<any>>(
  ...
  items.forEach(item => {
    ...
    result.push(val)
    if (val.hasChildren) result.push(...flatten(children, childrenField, level + 1))
  })
  return result
}
...
function updateTreeValue() {
  ...
  treeValue.value = Array.isArray(val) ? val.map(v => getItemByValue(v)) : getItemByValue(val)
}
...
function updateModelValue(val: Arrayable<Recordable<any>>) {
  modelValue.value = Array.isArray(val) ? val.map(v => get(v, props.valueField)) : get(val, props.valueField)
}
...
function expandAll() {
  expanded.value = flattenData.value.filter(item => item.hasChildren).map(item => get(item.value, props.valueField))
}
...
<template>
  ...
  >
    ...
    >
      ...
      <Checkbox v-if="multiple" :checked="isSelected" :indeterminate="isIndeterminate" @click.stop="handleSelect" />
      <div
